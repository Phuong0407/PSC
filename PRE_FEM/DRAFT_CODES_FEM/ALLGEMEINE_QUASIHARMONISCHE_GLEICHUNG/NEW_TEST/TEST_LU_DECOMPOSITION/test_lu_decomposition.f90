...
program generate_test_case
  ...
  real(8), allocatable :: A(:,:), B(:), SOLUTION(:)
  integer :: i, j
  real(8) :: random_value
  

  ! Allocate matrix and vector
  allocate(A(n, n), B(n), SOLUTION(n))
  ...
  do i = 1, n
      do j = 1, n
          ...
          A(i, j) = random_value * 10.0
      end do
      A(i, i) = A(i, i) + sum(abs(A(i, :)))  ! Make A diagonally dominant
  end do

  do i = 1, n
      call random_number(random_value)
      B(i) = random_value * 10.0
  end do

  ! Write matrix A and vector B to the console
  print *, "Matrix A:"
  do i = 1, n
    write(*, '(15F10.5)') A(i, :)
  end do
...
SOLUTION = SOLVE_DENSE_MATRIX_SYSTEM(A,B)

  print *, "Vector B:"
  write(*, '(15F10.5)') SOLUTION(:)
  ! Deallocate memory
  deallocate(A, B)
end program generate_test_case
