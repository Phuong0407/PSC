...
! GRID DATA GENERATION
    ...
    subroutine GEN_NODE_CONN()
        ...
        allocate(GRID%NODE_CONN(NUM_NODE, MAX_CONN_PER_NODE))
        CURRENT_NUM_CONN = 1
        GRID%NODE_CONN = 0
...
! LOCAL STIFFNESS MATRIX CALCULATION
    function CALC_AXISYM_LSM_LOC(z_1, r_1, z_2, r_2, z_3, r_3) result(STIFF_MAT_LOC)
        implicit none
        real(8), intent(in) :: z_1, r_1, z_2, r_2, z_3, r_3
        real(8), allocatable :: STIFF_MAT_LOC(:,:)
        real(8) :: c(3), d(3)
        real(8) :: A_elem, r_centroid
        integer :: i, j
...
! CALCULATE DIRICHLET BOUNDARY CONDITION
    subroutine CALC_DIRICHLET_BOUND()
        ...
        real(8) :: PSIUPPER, PSILEFT, PSIRIGHT, R_1, R_2, R_3
        ...
        do i = 2, GRID%eV
            ...
            R_2 = GRID%dy2 * (i - 1) + R_3
            PSIRIGHT = U_1 * (R_2 - R_3)
        ...
        do i = 1, GRID%nH
            ...
            DIRICHLET_BOUND(IND_UPPER_BOUND) = PSIUPPER
    ...
    subroutine CALC_DIRICHLET_BOUND_AXISYM()
        ...
        do i = 2, GRID%eV
            ...
            IND_RIGHT_BOUND = i * GRID%nH
            R_2 = GRID%dy2 * (i - 1) + R_3
            PSIRIGHT = 0.5 * U_1 * (R_2**2.0 - R_3**2.0)
            DIRICHLET_BOUND(IND_RIGHT_BOUND) = PSIRIGHT
            GRID%IND_DIRICHLET_BOUND(k) = IND_RIGHT_BOUND
            k = k + 1
        end do
...
!***************************************************************************
    subroutine INIT_SOLUTION(iH_1, iH_2, iH_3, iL_1, iL_2, iN, iM_1, iM_2, iM_3, iU_0)
        ...
        !     print *, "elem", i
        !     do j = 1, size(GRID%NODE_CONN,2)
        !         print *, GRID%NODE_CONN(i,j)
        !     end do
        ! end do
        ! call GEN_EDGE_CONN()
...
! PATCH RECOVERY OF STREAMLINE FUCNTION GRADIENT
    ...
    subroutine CALC_BOUND_NODE_GRAD_RECOVERY(BOUND_NODE_IND)
        ...
        ! PRINT *, "ANH EM CHUNG TA"
    end subroutine CALC_BOUND_NODE_GRAD_RECOVERY

    subroutine CALC_3_CORNER_NODE_GRAD_RECOVERY(CORNER_3_IND)
        implicit none
        integer, intent(in) :: CORNER_3_IND
        ...
        IND_FITTING_NODE = 0
        IND_FITTING_NODE(1) = CORNER_3_IND
        do i = 2, 4
            if (GRID%NODE_CONN(GRID%NODE_CONN(CORNER_3_IND, i), 8) == 7) then
                IND_INTERNAL_NODE = GRID%NODE_CONN(CORNER_3_IND, i)
                INTERNAL_NODE_COUNT = INTERNAL_NODE_COUNT + 1
            end if
            IND_FITTING_NODE(i) = GRID%NODE_CONN(CORNER_3_IND, i)
        end do
        ...
        NODE_GRAD_RECOVERY(CORNER_3_IND, 1) = X_DEV
        NODE_GRAD_RECOVERY(CORNER_3_IND, 2) = Y_DEV
        NODE_GRAD_RECOVERY(CORNER_3_IND, 3) = XX_DEV
        NODE_GRAD_RECOVERY(CORNER_3_IND, 4) = XY_DEV
        NODE_GRAD_RECOVERY(CORNER_3_IND, 5) = YY_DEV
    ...
    function CALC_LONGEST_LENGTH_OF_NODE_CONNECTED_TO_A_NODE(x_i, y_i, IND) result(MAX_DIST)
        ...
        integer :: i, NUMBER_OF_ADJACENT_POINTS
        real(8) :: DIST, MAX_DIST

        NUMBER_OF_ADJACENT_POINTS = size(x_i) - 1
        MAX_DIST = sqrt((x_i(2) - x_i(1))**2.0 + (y_i(2) - y_i(1))**2.0)
        IND = 2

        do i = 3, NUMBER_OF_ADJACENT_POINTS
