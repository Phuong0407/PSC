module csr_sparse_matrix
    implicit none
    private
    real :: ZERO_COMPARED_TOLERANCE, MATRIX_NORM    ! AXILIARY VARIABLE to check for NULL-INDICES in sparse matrix
    integer :: SIZE_MAT, NON_ZERO_ELEM              ! Size of matrix and number of non-zero elements
    real, allocatable :: VAL(:)                     ! Non-zero values
    integer, allocatable :: COL_IND(:)              ! Column indices of non-zero elements
    integer, allocatable :: ROW_PTR(:)              ! Row pointer array
    real :: CONV_TOL = 1.0E-6                       ! Tolerance for convergence
    real :: MAX_ITER = 100000                       ! Maximum number of iterations
...
contains
    subroutine INIT_SPARSE_SOLVER(SPARSE_MAT)
        implicit none
        real, intent(in), dimension(:,:) :: SPARSE_MAT
        integer :: i, j, index
    ...
    function SOLVE_SPARSE_SYSTEM(RHS_VECT) result(X)
        implicit none
        real, intent(in) :: RHS_VECT(:)
        real, allocatable :: r(:), p(:), Ap(:), X(:)
        real :: alpha, beta, rnorm, bnorm, tol
        integer :: ITER
